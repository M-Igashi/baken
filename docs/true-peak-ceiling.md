...
The native-lossless raise threshold (the True Peak below which an MP3/AAC file qualifies for in-place global_gain modification) is always `target − 1.5 dB`, since global_gain only works in 1.5 dB steps. A file closer to the ceiling than that is left where it is: the only way to raise it is a re-encode, and spending a lossy generation to move a file by less than one step is not worth it, least of all because that band is exactly where every file lands after a native step ([#138](https://github.com/M-Igashi/baken/issues/138)). Lowering never re-encodes either.
