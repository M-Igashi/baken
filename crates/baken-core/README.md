...
`headroom` (gain application, re-encode and the measurement fallback) and `cdjsafe` shell out to ffmpeg and ffprobe. They are looked up on `PATH` by default; call `baken_core::set_tools` to point at bundled binaries. Long-running operations take a `Progress` callback and a `CancelToken`.
